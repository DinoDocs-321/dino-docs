...
const JSONEditor = () => {
  ...
  //   const generatedJsonData = JSON.parse(jsonCode);  // Get the generated JSON data
  //   const schemaName = prompt("Enter a schema name:", "GeneratedSchema");

  //   axios.post('http://localhost:8000/api/save-json/', {
  //       schema_name: schemaName,  // Dynamically set schema name
  //       json_data: generatedJsonData  // Use the JSON from the editor
  //   }, {
  //       headers: {
  ...
  const handleSubmit = () => {
    ...
    const json_data = generateJSONData();
    const bodyParameters = {
        schema_name: schema_name,  // Schema name input
        json_data: json_data       // JSON data generated by your editor
    };

    axios.post('http://localhost:8000/api/save-json/', bodyParameters, config)
        .then(response => {
            console.log('Data saved successfully:', response.data);  // Handle success
        })
        .catch(error => {
            console.error('There was an error saving the data:', error.response ? error.response.data : error.message);  // Handle error
        });};
